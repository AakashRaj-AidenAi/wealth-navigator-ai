...
from app.models.communication import SentimentLog
...
async def list_alerts(
    ...
    query = query.order_by(ComplianceAlert.created_at.desc()).offset(skip).limit(limit)
...
async def list_churn_predictions(
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(50, ge=1, le=200, description="Max records to return"),
    churn_risk_percentage_gte: int = Query(
        None, description="Filter by minimum churn risk percentage"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    ...
    query = select(ChurnPrediction).where(
        ChurnPrediction.advisor_id == current_user.id
    )

    if churn_risk_percentage_gte is not None:
        query = query.where(
            ChurnPrediction.churn_risk_percentage >= churn_risk_percentage_gte
        )

    query = query.order_by(
        ChurnPrediction.churn_risk_percentage.desc().nullslast()
...
async def list_engagement_scores(
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(50, ge=1, le=200, description="Max records to return"),
    engagement_score_gte: int = Query(
        None, description="Filter by minimum engagement score"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
) -> list[EngagementScoreResponse]:
    """List client engagement scores for the current advisor."""
    query = select(ClientEngagementScore).where(
        ClientEngagementScore.advisor_id == current_user.id
    )

    if engagement_score_gte is not None:
        query = query.where(
            ClientEngagementScore.engagement_score >= engagement_score_gte
        )
...
@router.get("/sentiment-logs", response_model=list[SentimentLogResponse])
async def list_sentiment_logs(
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(50, ge=1, le=200, description="Max records to return"),
    sentiment_in: str = Query(
        None, description="Filter by sentiment labels (comma-separated)"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
) -> list[SentimentLogResponse]:
    """List sentiment logs for the current advisor."""
    query = select(SentimentLog).where(
        SentimentLog.advisor_id == current_user.id
    )

    if sentiment_in:
        sentiment_labels = [s.strip() for s in sentiment_in.split(",")]
        query = query.where(SentimentLog.sentiment_label.in_(sentiment_labels))

    query = query.order_by(SentimentLog.analyzed_at.desc()).offset(skip).limit(
        limit
    )

    result = await db.execute(query)
    logs = result.scalars().all()
    return [SentimentLogResponse.model_validate(log) for log in logs]
