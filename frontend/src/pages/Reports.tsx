...
const Reports = () => {
  ...
  const fetchStats = async () => {
    ...
    try {
      const [allRaw, monthlyRaw] = await Promise.all([
        api.get('/reports'),
        api.get('/reports', { created_after: startOfMonth })
      ]);
      const allReports = extractItems(allRaw);
      const monthlyReports = extractItems(monthlyRaw);

      const totalCount = allReports.length;
      const monthCount = monthlyReports.length;
