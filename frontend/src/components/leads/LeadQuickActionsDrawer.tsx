...
export const LeadQuickActionsDrawer = ({ 
  ...
  const handleTabChange = async (value: string) => {
    ...
    if (value === 'activity') {
      try {
        const data = extractItems<LeadActivity>(await api.get('/lead_activities', { lead_id: lead.id }));
        setActivities(data);
      } catch { /* API client shows toast */ }
    } else if (value === 'history') {
      setLoadingHistory(true);
      try {
        const data = extractItems(await api.get('/lead_stage_history', { lead_id: lead.id }));
