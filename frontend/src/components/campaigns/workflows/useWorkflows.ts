...
export function useWorkflows() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const data = extractItems<AutomationWorkflow>(await api.get('/automation_workflows'));
      return data;
    },
    enabled: !!user,
  });
...
export function useWorkflowSteps(workflowId: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      const data = extractItems<WorkflowStep>(await api.get('/workflow_steps', { workflow_id: workflowId! }));
      return data;
    },
    enabled: !!workflowId,
  });
...
export function useWorkflowLogs(workflowId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const data = extractItems<WorkflowLog>(await api.get('/workflow_logs', params));
      return data;
    },
  });
}
