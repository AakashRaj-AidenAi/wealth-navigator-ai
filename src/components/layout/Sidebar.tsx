...
export const Sidebar = ({ expanded, onToggle, mode, onOpenChat, onCloseChat }: SidebarProps) => {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <aside
        className={cn(
          'fixed left-0 top-0 z-40 h-screen bg-sidebar border-r border-sidebar-border flex flex-col transition-all duration-200 ease-in-out overflow-hidden',
          expanded ? 'w-64' : 'w-16'
        )}
      >
        {/* Header: Logo + Toggle */}
        ...
        {/* Bottom section: AI Copilot toggle + User profile */}
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  onClick={onOpenChat}
                  className={cn(
                    'w-full flex items-center gap-3 px-3 py-2.5 transition-colors hover:bg-sidebar-accent text-sidebar-foreground',
